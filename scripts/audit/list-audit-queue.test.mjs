...
describe('checked-in sovereign baseline', () => {
  it('contains the exact 197-entry Chinese MFA baseline and only evidence-backed verified countries', async () => {
    ...
    expect(registry.countries.filter((entry) => entry.status === 'verified').map((entry) => entry.sovereignCode)).toEqual(['AD', 'LI']);
    expect(registry.countries.filter((entry) => !['AD', 'LI'].includes(entry.sovereignCode)).every((entry) => entry.status === 'draft')).toBe(true);
  });
