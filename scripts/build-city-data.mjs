...
export async function buildCityData({ citiesPath, aliasesPath, countriesPath, outputPath, sourceDate }) {
  ...
  await forEachLine(citiesPath, (line, lineNumber) => {
    ...
    const chineseAliasSet = new Set(chinese?.aliases ?? []);
    const sourceAliases = uniqueStrings(aliasesText ? aliasesText.split(',') : []).filter(
      (alias) => alias !== name && alias !== asciiName && !chineseAliasSet.has(alias),
    );
    if (sourceAliases.length > ALIAS_LIMIT) truncatedAliasCities += 1;
    const chineseAliases = (chinese?.aliases ?? []).filter(
      (alias) => alias !== chinese?.preferred && alias !== name && alias !== asciiName,
    );
    const aliases = uniqueStrings([...sourceAliases.slice(0, ALIAS_LIMIT), ...chineseAliases]);
    rows.push([
      id,
      name,
      asciiName,
  ...
  const source = renderModule(rows, sourceDate, checksum, truncatedAliasCities);
  await mkdir(dirname(outputPath), { recursive: true });
  await writeFile(outputPath, source, 'utf8');
  return { cityCount: rows.length, checksum, truncatedAliasCities };
}
...
async function readChineseNames(path) {
  ...
  await forEachLine(path, (line, lineNumber) => {
    ...
    if (!name) throw new Error(`Malformed Chinese alternate name at line ${lineNumber}`);
...
function renderModule(rows, sourceDate, checksum, truncatedAliasCities) {
  return `// Generated by scripts/build-city-data.mjs. Do not edit manually.\n` +
    ...
    `export const CITY_DATA_CITY_COUNT = ${rows.length};\n` +
    `export const CITY_DATA_ALIAS_LIMIT = ${ALIAS_LIMIT};\n` +
    `export const CITY_DATA_ALIAS_POLICY = "Original name and ASCII name are always retained; up to ${ALIAS_LIMIT} source-order aliases plus every zh/zh-CN name are retained.";\n` +
    `export const CITY_DATA_TRUNCATED_ALIAS_CITIES = ${truncatedAliasCities};\n` +
